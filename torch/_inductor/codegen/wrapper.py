...
class EnterCudaStreamContextLine(WrapperLine):
    ...
    (Python ``with torch.cuda.stream(...)`` vs C++ ``CUDAStreamGuard``).
    """
...
class PythonWrapperCodegen(CodeGen):
    ...
    def codegen_enter_cuda_stream_context(
        ...
        code.writeline(f"with torch.cuda.stream({get_stream_name(stream_idx)}):")
        code.do_indent()
    ...
    def codegen_stream_info_prologue(
        ...
        if num_streams > 1:
            if any(
                i in stream_idx_to_user_obj_idx for i in range(1, num_streams)
            ):
                import_line = (
