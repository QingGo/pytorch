...
class GridExpr:
    ...
    def generate_lazy(self, kernel_result_name: str) -> None:
        ...
        """
        meta: dict[str, Any] = {
            "XBLOCK": f"{kernel_result_name}.xblocks[0]",
            "YBLOCK": f"{kernel_result_name}.yblocks[0]",
            "ZBLOCK": f"{kernel_result_name}.zblocks[0]",
            "R0_BLOCK": f"{kernel_result_name}.r0blocks[0]",
...
class SequentialFlattenComboKernelGrid(GridExpr):
    ...
    def generate_lazy(self, kernel_result_name: str) -> None:
        ...
        meta: dict[str, Any] = {}
        for i in range(num_kernels):
            meta[f"XBLOCK_{i}"] = f"{kernel_result_name}.xblocks[{i}]"
            meta[f"YBLOCK_{i}"] = f"{kernel_result_name}.yblocks[{i}]"
        self.generate(meta, is_lazy=True)
